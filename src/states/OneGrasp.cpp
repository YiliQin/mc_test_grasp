...
void OneGrasp::computeTargetRelative()
{
  ...
  target_.translation() = Eigen::Vector3d(depth_, 
              -target_pos_.x()+opposite_hand_pose_.translation().y(), 
              target_pos_.y()+opposite_hand_pose_.translation().z());
  //target_.rotation() = sva::RotX(M_PI*target_pos_.z()/180);
  if (active_hand_ == "Left") 
    target_.rotation() = opposite_hand_pose_.rotation()*sva::RotX(mc_rtc::constants::PI*target_pos_.z()/180);
  else if (active_hand_ == "Right")
    target_.rotation() = opposite_hand_pose_.rotation()*sva::RotX(mc_rtc::constants::PI*(1-target_pos_.z()/180));
  else ;
  pre_target_ = target_;
  pre_target_.translation().x() -= approach_depth_;
}
